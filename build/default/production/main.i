...
static const float DATA_OFFSET = 63550;
static const float DATA_MULT = -15.6863;
...
ppmData.reg[0] = 0;
for(uint8_t i = 1; i < ppmData.ppmRegSize; i++) {
ppmData.reg[i] = 0x7F;
}
ppmData.iReg = 0;
}
...
ppmData.reg[i] = uartData.buf[i+1]*DATA_MULT + DATA_OFFSET;
}
}

bool CheckCRC() {
uint8_t inc = 0;
for(uint8_t i = 0; i < uartData.uartBufSize - 1; i++) {
for(uint8_t j = 0; j < 8; j++) {
inc = inc + ((uartData.buf[i] >> j)&0x01);
}
}
if(inc == uartData.buf[uartData.uartBufSize - 1]) return 1;
else return 0;
}
...
if(uartData.buf[0] > (20 - (1 + 1))) uartLoadState = UART_READY;
else {

uartData.uartBufSize = uartData.buf[0] + (1 + 1);
